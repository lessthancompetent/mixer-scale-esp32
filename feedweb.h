...
public:
    ...
    void begin() {
        ...
        server.on("/loading",           HTTP_GET,    [this]{ handleLoadingUI(); });
    ...
    void handleLoadingUI() {
        // Standalone mobile loading page — 192.168.4.1/loading
        // No nav, no settings. Pure tap-to-load UX.
        String h = F("<!DOCTYPE html><html><head>"
            "<meta charset='UTF-8'>"
            "<meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no'>"
            "<title>Loading</title><style>"
            "*{box-sizing:border-box;margin:0;padding:0}"
            "body{background:#0f1117;color:#e2e4ef;"
              "font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;"
              "min-height:100vh;padding-bottom:76px}"
            "#hdr{position:sticky;top:0;background:#1a1d27;"
              "border-bottom:1px solid #2a2d3d;padding:12px 16px;"
              "display:flex;align-items:center;justify-content:space-between;z-index:10}"
            "#herd-lbl{font-size:0.8rem;color:#6b7280;margin-bottom:2px}"
            "#scale-row{display:flex;align-items:baseline;gap:6px}"
            "#scale-val{font-size:2.2rem;font-weight:700}"
            "#scale-unit{font-size:0.9rem;color:#6b7280}"
            "#scale-badge{font-size:0.75rem;padding:4px 10px;border-radius:20px;"
              "background:rgba(34,197,94,0.15);color:#22c55e;white-space:nowrap}"
            "#scale-badge.err{background:rgba(239,68,68,0.15);color:#ef4444}"
            "#no-sess{padding:40px 16px;text-align:center;display:none}"
            "#no-sess p{color:#6b7280;margin-bottom:16px}"
            "#no-sess a{color:#3b82f6;font-size:0.9rem}"
            "#steps{padding:12px;display:flex;flex-direction:column;gap:10px}"
            ".sc{background:#1a1d27;border:2px solid #2a2d3d;border-radius:12px;"
            ...
            ".sc.done{border-color:#166534;background:rgba(22,101,52,0.12);opacity:0.65;cursor:default}"
            ".sc.cur{border-color:#3b82f6;background:rgba(59,130,246,0.08)}"
            ".sc.hit{border-color:#ef4444;background:rgba(239,68,68,0.1);"
              "animation:pulse 0.55s ease-in-out infinite alternate}"
            "@keyframes pulse{from{background:rgba(239,68,68,0.06)}to{background:rgba(239,68,68,0.22)}}"
            ".sc-name{font-size:1.2rem;font-weight:700;margin-bottom:3px}"
            ".sc-lbl{font-size:0.78rem;color:#6b7280;margin-bottom:10px}"
            ".sc.cur  .sc-lbl{color:#3b82f6}"
            ".sc.hit  .sc-lbl{color:#ef4444;font-weight:600}"
            ".sc.done .sc-lbl{color:#22c55e}"
            ".pb-bg{background:#0f1117;border-radius:6px;height:14px;overflow:hidden;margin-bottom:8px}"
            ".pb{height:100%;border-radius:6px;background:#3b82f6;transition:width 0.3s}"
            ".sc.hit  .pb{background:#ef4444}"
            ".sc.done .pb{background:#22c55e}"
            ".sc-wt{display:flex;justify-content:space-between;font-size:0.82rem;color:#6b7280}"
            ".sc-wt b{color:#e2e4ef}"
            "#footer{position:fixed;bottom:0;left:0;right:0;"
              "padding:12px;background:#0f1117;border-top:1px solid #2a2d3d}"
            ".btn{width:100%;padding:15px;border:none;border-radius:10px;"
              "font-size:1rem;font-weight:700;cursor:pointer;color:#fff}"
            "#btn-start{background:#1e3a5f}"
            "#btn-finish{background:#22c55e}"
            "</style></head><body>"
            "<div id='hdr'>"
              "<div>"
                "<div id='herd-lbl'>Mixer Wagon</div>"
                "<div id='scale-row'>"
                  "<span id='scale-val'>—</span>"
                  "<span id='scale-unit'>kg</span>"
                "</div>"
              "</div>"
              "<span id='scale-badge'>Scale</span>"
            "</div>"
            "<div id='no-sess'>"
              "<p>No mix loaded.</p>"
            ...
            "<div id='steps'></div>"
            "<div id='footer'>"
              "<button id='btn-start'  class='btn' onclick='startLoad()' style='display:none'>&#x25B6; Start Load</button>"
              "<button id='btn-finish' class='btn' onclick='finishLoad()' style='display:none'>&#x2713; Finish Mix</button>"
            "</div>"
            "<script>"
            "var ings=[],st={},poll;"
            "async function api(u,m){"
              "try{var r=await fetch(u,{method:m||'GET'});return r.ok?r.json():null;}catch{return null;}"
            "}"
            "async function init(){"
              "var r=await api('/api/ingredients');"
              "if(r&&r.ingredients)ings=r.ingredients;"
              "poll=setInterval(refresh,500);refresh();"
            "}"
            "async function refresh(){"
              "var d=await api('/api/status');if(!d)return;st=d;render(d);"
            "}"
            "function render(d){"
              "document.getElementById('scale-val').textContent=(d.scale_kg||0).toFixed(1);"
              "var b=document.getElementById('scale-badge');"
              "b.textContent=d.scale_ok?'Scale OK':'Scale ERR';"
              "b.className=d.scale_ok?'':'err';"
              "var ns=document.getElementById('no-sess');"
              "var sc=document.getElementById('steps');"
              "var bs=document.getElementById('btn-start');"
              "var bf=document.getElementById('btn-finish');"
              "if(!d.session_active){"
                "ns.style.display='block';sc.innerHTML='';"
                "bs.style.display='block';bf.style.display='none';return;"
              "}"
              "ns.style.display='none';bs.style.display='none';"
              "var steps=d.steps||[];"
              "var allDone=steps.length>0&&steps.every(function(s){return s.complete;});"
              "bf.style.display=allDone?'block':'none';"
              "var html='';"
              "for(var i=0;i<steps.length;i++){"
                "var s=steps[i];"
                "var nm=(ings[s.ingredient_idx]||{name:'Feed '+(s.ingredient_idx+1)}).name;"
                "var isCur=(i===d.current_step);"
                "var pct=s.target_wet_kg>0?Math.min(100,s.loaded_wet_kg/s.target_wet_kg*100):0;"
                "var hit=isCur&&!s.complete&&s.loaded_wet_kg>=s.target_wet_kg;"
                "var cls=s.complete?'done':(hit?'hit':(isCur?'cur':''));"
                "var lbl=s.complete?'&#x2713; Done':"
                          "(hit?'&#x26A1; Target reached &mdash; tap to confirm':"
                          "(isCur?'&#x25CF; Loading now':'Tap to start loading'));"
                "html+='<div class=\"sc '+cls+'\" onclick=\"tap('+i+')\">';"
                "html+='<div class=\"sc-name\">'+nm+'</div>';"
                "html+='<div class=\"sc-lbl\">'+lbl+'</div>';"
                "html+='<div class=\"pb-bg\"><div class=\"pb\" style=\"width:'+pct.toFixed(0)+'%\"></div></div>';"
                "html+='<div class=\"sc-wt\">';"
                "html+='<span>Loaded: <b>'+s.loaded_wet_kg.toFixed(0)+' kg</b></span>';"
                "html+='<span>Target: <b>'+s.target_wet_kg.toFixed(0)+' kg</b> ('+pct.toFixed(0)+'%)</span>';"
                "html+='</div></div>';"
              "}"
              "sc.innerHTML=html;"
            "}"
            "async function tap(i){"
              "var steps=st.steps||[];if(!steps[i]||steps[i].complete)return;"
              "var isCur=(i===st.current_step);"
              "var hit=isCur&&steps[i].loaded_wet_kg>=steps[i].target_wet_kg;"
              "await api(hit?'/api/load/confirm':'/api/load/activate?step='+i,'POST');"
              "refresh();"
            "}"
            "async function startLoad(){await api('/api/load/start','POST');refresh();}"
            "async function finishLoad(){await api('/api/load/finish','POST');refresh();}"
            "init();"
            "</script></body></html>"
        );
